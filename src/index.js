...
class Mcwss extends events_1.default {
    server;
    clients = {};
    config;
    constructor(config) {
    ...
    register() {
        this.on('connection', (data) => {
            if (this.config.autoClearEvents)
                const_1.ALL_EVENTS.forEach((eventName) => data.client.unsubscribe(eventName));
            this.eventStack.forEach((el) => {
                const handle = el.type
                    .split('_')
                    .map((str) => `${str.charAt(0).toUpperCase()}${str.slice(1)}`)
                    .join('');
                if (const_1.ALL_EVENTS.includes(handle))
                    this.config.events.add(handle);
            });
    ...
    handle(data) {
        try {
            ...
            events_2.eventDataMinecraftBaseSchema.parse(handle);
            this.emit(handle.header.eventName
                .replace(/([a-z])([A-Z])/g, '$1_$2')
                .toLowerCase()
                .replace(/[-\s]/g, '_'), { ...handle, client: data.client });
        }
        catch (e) {
            if (e instanceof tsukiko_1.TsuError) {
    ...
    start() {
        ...
        this.server?.on('connection', (ws, req) => {
            ...
            const client = new client_1.Client(ws, req, id);
...
mcwss.on('ready', () => {
    logger_1.default.info(`WebSocketServer started at ws://localehost:${PORT} `);
});
mcwss.on('dispose', () => logger_1.default.info('WebSocketServer stopped'));
mcwss.on('error', (data) => logger_1.default.error(data.client ? `[Client:${data.client.sessionId}]` : '[Server]', data.error.name, data.error.message));
mcwss.on('connection', (data) => logger_1.default.info(`[Client:${data.client.sessionId}]`, 'new connection from', data.client.req.socket.remoteAddress));
mcwss.on('close', (data) => {
    if (data.raw) {
        logger_1.default.info(`[Client:${data.raw.client.sessionId}]`, 'Closing in progress code:', data.raw.code);
        return;
    }
    logger_1.default.info('WebSocketServer is closeing...');
});
process.stdin.on('data', (data) => {
    ...
    if (message in matchList)
        matchList[message]();
});
mcwss.on('block_broken', (data) => {
    const { block, player, tool } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)(`玩家 %name% 在 %position% 处破坏了方块 %block% %tool%`, {
        name: player.name,
...
mcwss.on('block_placed', (data) => {
    const { block, player, tool } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处放置了方块 %block% 通过工具 %tool%', {
        name: player.name,
        position: Object.values(player.position).join(','),
        block: `${block.namespace}:${block.id}`,
        tool: tool.id ? `${tool.namespace}:${tool.id}` : '' ?? '无'
...
mcwss.on('end_of_day', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处刚经历了黑夜', {
        name: player.name,
        position: Object.values(player.position).join(',')
    }));
});
mcwss.on('entity_spawned', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处生成了 %type% 类型的生物实体', {
        name: player.name,
        position: Object.values(player.position).join(','),
        type: mob.type
    }));
...
mcwss.on('item_acquired', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处获得了 %count% 个 %item%', {
        name: player.name,
        position: Object.values(player.position).join(','),
        count,
...
mcwss.on('item_crafted', (data) => {
    const { player, item, count } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处合成了 %count% 个 %item%', {
        name: player.name,
        position: Object.values(player.position).join(','),
        count,
        item: `${item.namespace}:${item.id}`
...
mcwss.on('item_destroyed', (data) => {
    const { player, item, count } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处破坏了 %count% 个 %item%', {
        name: player.name,
        position: Object.values(player.position).join(','),
        count,
        item: `${item.namespace}:${item.id}`
...
mcwss.on('item_smelted', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处熔炼了 %item%', {
        name: player.name,
        position: Object.values(player.position).join(','),
        item: `${item.namespace}:${item.id}`
    }));
...
mcwss.on('item_used', (data) => {
    const { player, item, count } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处使用了 %count% 个 %item%', {
        name: player.name,
        position: Object.values(player.position).join(','),
        count,
        item: `${item.namespace}:${item.id}`
...
mcwss.on('jukebox_used', (data) => {
    const { player, item, count } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处播放了 %count% 个 %item% 唱片', {
        name: player.name,
        position: Object.values(player.position).join(','),
        count,
        item: `${item.namespace}:${item.id}`
...
mcwss.on('mob_interacted', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处与 %mob% 发生了 %type% 交互', {
        name: player.name,
        position: Object.values(player.position).join(','),
...
mcwss.on('mob_killed', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处杀死了在 %mob_position% 处的 %mob_id%（%mob_name%）', {
        name: player.name,
        position: Object.values(player.position).join(','),
        mob_id: victim.id,
...
mcwss.on('player_bounced', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处被 %block% 弹起', {
        name: player.name,
        position: Object.values(player.position).join(','),
        block: `${block.namespace}:${block.id}`
    }));
...
mcwss.on('player_died', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处被 %killer% 杀死', {
        name: player.name,
        position: Object.values(player.position).join(','),
...
mcwss.on('player_message', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('%sender% 对 %receiver% 发送了 %type% 消息：%message%', {
        sender,
...
mcwss.on('player_teleported', (data) => {
    ...
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处传送了 %meters% 格', {
        name: player.name,
        position: Object.values(player.position).join(','),
...
mcwss.on('player_travelled', (data) => {
    const { player, metersTravelled } = data.body;
    logger_1.default.log((0, tools_1.stringTemp)('玩家 %name% 在 %position% 处移动了 %meters% 格', {
        name: player.name,
        position: Object.values(player.position).join(','),
        meters: metersTravelled
    }));
    ...
    switch (data.body.message) {
        ...
        default:
            if (data.body.message.startsWith('./')) {
                ...
                const file = (0, path_1.join)(process.cwd(), data.body.message.split('*/func ')[1]);
                const result = data.client.func(file);
                if (result)
                    return;
                data.client.chatf((0, tools_1.stringTemp)('mcfunction 执行失败： 不存在的文件 %file%', { file }), "\u00A7c" /* TextColor.RED */);
            }
            break;
    }
});
